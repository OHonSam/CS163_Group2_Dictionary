...
void EndApp(App *app)
{
	...
	delete app->state.user;
	app->state.user = nullptr;
...
void FirstScreen::Render(App *app)
{
	...
	if (app->state.user != nullptr)
	{
		delete app->state.user;
		app->state.user = nullptr;
	}

	std::cout << "GROUP 1 COURSE MANAGEMENT SYSTEM\n\n";
	...
	while (!CheckString(buffer, choice) || choice < 0 || choice > 1)
	{
...
void LoginScreen::Render(App *app)
{
	...
	app->state.num = choice;

	ClearScreen();
	std::cout << "Your choice was: " << app->state.num << "\n";
	u->InputUsername();
	u->InputPassword();
	while (!u->CheckUser())
	{
...
void SYChoice::Render(App *app)
{
	...
	std::cout << "Your choice was: " << app->state.num << "\n\n";
	std::cout << "What would you like to do?\n";
	std::cout << "1. Work on classes\n";
	std::cout << "2. Work on semesters\n";
